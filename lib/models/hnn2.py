...
class HNN2(nn.Module):
    def __init__(self):
        ...
        if opt.vgg is not None:
            input_dim = 3
        self.conv1_1 = nn.Conv2d(input_dim, 64, 3, padding=35)### Todo padding? 35?
    ...
    def forward(self, x):
        ...
        upsample2 = torch.nn.functional.conv_transpose2d(score_dsn2, self.weight_deconv2.cuda(), stride=2)
        upsample3 = torch.nn.functional.conv_transpose2d(score_dsn3, self.weight_deconv3.cuda(), stride=4)
        upsample4 = torch.nn.functional.conv_transpose2d(score_dsn4, self.weight_deconv4.cuda(), stride=8)
        upsample5 = torch.nn.functional.conv_transpose2d(score_dsn5, self.weight_deconv5.cuda(), stride=16)
